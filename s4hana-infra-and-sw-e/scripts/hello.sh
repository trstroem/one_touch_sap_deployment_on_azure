...
echo  >> /home/juser/souvenir/scenarios/s4hana-pas-single-node-full-e/dev/ansible/roles/s4-install/templates/inifile.params
echo 'NW_HDB_getDBInfo.dbhost = '$hanaIp >> /home/juser/souvenir/scenarios/s4hana-pas-single-node-full-e/dev/ansible/roles/s4-install/templates/inifile.params
...
then
az deployment group create  --resource-group $5 --template-uri "https://raw.githubusercontent.com/trstroem/one_touch_sap_deployment_on_azure/master/s4hana-infra-and-sw-e/azure-bastion-deploy.json" --parameters location=$6 bastionHostName='s4bastion' subnetName='AzureBastionSubnet' publicIpAddressName='s4bastionpublicip' existingVNETName=$7 subnetAddressPrefix=$13
fi
az deployment group create  --resource-group $5 --template-uri "https://raw.githubusercontent.com/trstroem/one_touch_sap_deployment_on_azure/master/s4hana-infra-and-sw-e/jumpbox-windows-deploy.json" --parameters adminUsername='juser' adminPassword='Hana@123456' windowsOSVersion='2016-Datacenter' vmSize='Standard_D2_v3' location=$6 vNetName=$7 subnetName=$8
